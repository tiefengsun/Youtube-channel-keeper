...
双击 `start.cmd`，会自动创建 `.venv`、安装 Python 依赖并启动服务。或者在项目目录运行：
...
Windows 安装后双击 **`start.cmd`**，浏览器会打开 **http://127.0.0.1:8765**。Ubuntu 安装后由 `systemd --user` 自动启动服务；使用服务器本机地址、局域网地址或上文的 SSH 隧道地址访问。
