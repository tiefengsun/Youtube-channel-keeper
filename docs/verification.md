...
最近验证：2026-09-18，Windows，Python 3.12.7、Node.js 22.19.0、yt-dlp 2026.8.19。

## 已通过

- `python -m pytest -q`：**90 passed**。覆盖核心调度/API、定时运行、Cookies 导入、跨平台目录浏览、并发目录命名、会话登录、跨队列公平领取、后台线程恢复、解析并发限制、扫描深度，以及 5 个真实 yt-dlp + FFmpeg 媒体下载/转换测试。
- `node --check app/static/app.js`、`node --check app/static/login.js`：JavaScript 语法检查通过。
...
- Ubuntu 24.04 服务器曾于 2026-09-17 验证 systemd 用户服务及依赖；本次优化尚未同步到服务器或重新验证服务器端行为。
